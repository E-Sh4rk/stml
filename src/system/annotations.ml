...
module FullAnnot = struct
  ...
  let a = match a with
  | ConstA | AliasA | LetA | AbstractA | TupleA _ | ConsA _ | AppA _ | ProjA _
...
module PartialAnnot = struct
  ...
  let rec apply_subst_aux s =
    if Subst.is_identity s then ((fun t -> (t,false)), (fun a -> (a,false)))
      ...
      let apply_typ t =
        ...
        then t else (change := true ; Subst.apply s t)
      and apply_subst_a a =
      ...
      let apply_a a = match a with
        ...
        | TypA -> TypA
        | UntypA -> UntypA
        | ThenVarA -> ThenVarA | ElseVarA -> ElseVarA | ConstrA -> ConstrA
      ...
      and apply t =
        ...
        | Propagate (a, envs, t) ->
          Propagate (apply_subst_a a, List.map (Env.apply_subst s) envs, apply_subst_union t)
  ...
  let rec equals_a a1 a2 =
    ...
    | ThenVarA, ThenVarA | ElseVarA, ElseVarA | ConstrA, ConstrA -> true

...
let fold ef af =
  let rec aux_a a =
    begin match a with
    | Alias _ | Abstract _ | Const _ | App _ | Tuple _
...
let convert_to_msc ast =
  let aux expr_var_map ast =
    let rec to_defs_and_a expr_var_map ast =
      ...
      | Ast.Tuple es ->
        let aux (defs, expr_var_map, xs) e =
          let (defs', expr_var_map, x) = to_defs_and_x expr_var_map e in
          (defs'@defs, expr_var_map, x::xs)
        in
        let (defs, expr_var_map, xs) = List.fold_left aux ([], expr_var_map, []) es in
        (defs, expr_var_map, Tuple (List.rev xs))
      | Ast.Cons (e1, e2) ->
        let (defs1, expr_var_map, x1) = to_defs_and_x expr_var_map e1 in
        let (defs2, expr_var_map, x2) = to_defs_and_x expr_var_map e2 in
        (defs2@defs1, expr_var_map, Cons (x1, x2))
      | Ast.Projection (p, e) ->
        let (defs, expr_var_map, x) = to_defs_and_x expr_var_map e in

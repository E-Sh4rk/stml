...
let rec infer_mono_a vardef tenv expl env pannot_a a =
  ...
  let rec aux pannot_a a =
    ...
    | Tuple vs, InferA ->
      ...
      | None -> Ok TypA
      | Some v -> needvar v InferA UntypA
      end
    | Cons (v1, v2), InferA ->
      if memvar v1 |> not
      then needvar v1 InferA UntypA
      ...
      then needvar v2 InferA UntypA
        ...
        let res = tallying_infer [(vartype v2, list_typ)] in
    ...
    | Projection (p, v), InferA ->
      if memvar v then
        ...
        let res = tallying_infer ~noprio:[alpha] [(t, s)] in
        res |> List.iter (fun s ->
    ...
    | App (v1, v2), InferA ->
      ...
      then needvar v2 InferA UntypA
        ...
        let res = tallying_infer ~noprio:[alpha] [(t1, arrow_type)] in
        res |> List.iter (fun s ->
          log ~level:3 "Solution: %a@." Subst.pp s
        ) ;
        let res = simplify_tallying_infer env (TVar.typ alpha) res in
    ...
    | Lambda (ts, _, _), InferA ->
      let pannot_a_s = List.map (fun t ->
        let s = refresh (vars t |> TVarSet.filter TVar.can_infer) in
...
and infer_mono tenv expl env pannot e =
  ...
  let rec aux pannot e =
    ...
    | Bind (v,_,_), Propagate (pannot_a, envs, union) ->
      let propagate =
        envs |> Utils.find_among_others (fun env' _ -> is_compatible env env')
      in
      begin match propagate with
      | Some (env',envs) ->
        log ~level:1 "Var %a is ok but a split needs to be propagated.@." Variable.pp v ;
        let pannot1 = Keep (pannot_a, union) in
        let pannot2 = Propagate (pannot_a, envs, union) in
    ...
    | Bind (v, a, e), Keep (pannot_a, splits) ->
      ...
      let rec aux_splits splits =
        match splits with
        | ([],[],u) -> aux_splits ([(empty, Infer)],[],u)
        | ([],d,u) -> Ok (Keep (pannot_a, ([],d,u)))
        | ((s,_)::ex,d,u) when non_empty s && is_empty_poly (cap t s) -> aux_splits (ex,d,s::u)
        | ((s,pannot)::ex,d,u) ->
          let t = cap t s in
...
let typeof_infer tenv env e =
  ...
  typeof_nofail tenv env annot e |> simplify_typ
